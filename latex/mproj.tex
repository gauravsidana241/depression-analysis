...
By processing acoustic frames sequentially, Recurrent Neural Networks (RNNs) map the temporal evolution of a speaker's voice and preserve the contextual links between spoken segments and pauses. This capability makes them ideal for detecting the paralinguistic markers associated with depression. To model these temporal dynamics, this work adopts a stacked RNN architecture following Alsofyani and Vinciarelli \cite{alsofyani2021}. Stacking RNN layers allows deeper layers of the network to extract increasingly abstract features from the audio, while helping to manage the gradient instability common in long sequences: rather than relying on this depth alone, the input is also split into fixed-length segments \cite{pascanu2013}, which stabilizes backpropagation without requiring heavier, more expensive gating mechanisms like LSTMs or GRUs. Alsofyani and Vinciarelli \cite{alsofyani2021} recently validated this exact framework for student attachment classification. However, the architecture has not yet been benchmarked against static baselines or applied to depression detection.

\section{The Androids Corpus and Prior Benchmarks}\label{sec:corpus-survey}

The primary dataset used in this dissertation is the Androids Corpus, introduced by Tao, Esposito, and Vinciarelli \cite{tao2023androids}. It includes audio recordings for a reading task and an interview task collected from participants across mental health centers in Southern Italy. The interview task was specifically selected for model training because spontaneous speech more reliably captures the natural prosodic and paralinguistic variations associated with depression, which are often masked or suppressed during constrained reading tasks. This selected subset comprises recordings from 116 participants, of which 64 are individuals diagnosed with depression. This study utilizes the speaker-independent k-fold splits provided with the corpus to ensure fair cross-study evaluation. Multiple architectures have previously been tested on these specific splits, with Alsarrani et al.\ \cite{alsarrani2025} compiling F1 and accuracy results from these and other approaches in a comparative summary. To contextualize these results, a Support Vector Machine trained on whole-recording averages is also included as a reference point against the corpus baseline.

\section{The Gap}\label{sec:gap}

The literature reviewed above reveals a consistent pattern across both modalities of speech-based depression detection. Acoustic markers of depression are well documented, spanning voice quality, spectral structure, and timing (Section 2.1), yet the classifiers built on these markers overwhelmingly reduce each recording, or a fixed window of it, to a single static representation (Section 2.2). This holds even though speech is inherently sequential, and even though temporal architectures capable of modeling that sequence have already been validated for a closely related paralinguistic task: attachment recognition from children's spontaneous speech \cite{alsofyani2021}. No prior work has directly tested whether this advantage extends to depression detection, or whether it depends on how much temporal context, in the form of segment length, the classifier is given.

This question is related to, but distinct from, one addressed by Alsarrani et al.\ \cite{alsarrani2025}, who ask whether depression traces in speech are punctual or continuous by comparing Single Instance Learning against Multiple Instance Learning. Their comparison concerns how instance-level predictions within a segment are aggregated: Multiple Instance Learning allows a minority of depression-indicative feature vectors to determine a segment's label, while Single Instance Learning requires a majority. This is a question about weak-label instance selection, not about whether the classifier itself models time. In contrast, this dissertation asks whether providing a classifier with sequential memory, an architectural property rather than an aggregation rule, improves depression detection over static classifiers, and how this interacts with segment length.

To address this gap, this dissertation directly compares a stacked Recurrent Neural Network against static Logistic Regression and Support Vector Machine baselines, evaluated across a systematic range of segment lengths on the Androids Corpus.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Architecture}\label{architecture}

% TODO (author's note from draft): This section needs the most polishing -- it
% currently touches on code topics and does not explain the underlying mechanism
% in enough detail, such as AdamW and binary cross-entropy.
...
\begin{figure}[htbp]
  ...
  \includegraphics[width=0.92\textwidth]{figures/f1_score_vs_segment_length_full_interview_recording}
  \caption{F1 score as a function of segment length, for the participant-only
  condition (top) and the full interview recordings (bottom). Error bars show
  the standard deviation over the five folds. The Support Vector Machine and
  random baseline are shown as horizontal lines, since neither depends on
  segment length.}
  \label{fig:f1-segment-length}
\end{figure}
...
\begin{table}[htbp]
  \centering
  \caption{Results reported on the Androids Corpus. Not all works state whether
  they use the folds provided with the data, or which audio condition they use,
  but the values still provide a reference for comparison.}
  \label{tab:published}
  \begin{tabular}{lcc}
    \toprule
    ...
    Ilias and Askounis [XX] & $95.3 \pm 4.2$  & $95.8 \pm 3.8$ \\
    Ntalampiras [XX]        & $87.0 \pm 1.2$  & $88.9 \pm 1.7$ \\
    Phukan et al.\ [XX]     & 87.9            & 83.3 \\
    Yu and Kaya [XX]        & $88.2 \pm 8.5$  & $86.3 \pm 11.5$ \\
    \midrule
...
All results reported so far use the participant-only condition. This section repeats the same experiments on the full interview recordings, which additionally contain the interviewer's speech, to determine whether the comparison between temporal and static classification depends on which audio is used. Table~\ref{tab:best-configs} reports the best configuration of each model under both conditions, and Figure~\ref{fig:f1-segment-length} (bottom) shows the variation of F1 score with segment length for the full recordings.
...
This dissertation investigated whether a temporal classifier detects depression from speech more reliably than static classifiers, and whether any such advantage depends on the amount of speech supplied. A stacked Recurrent Neural Network, adapted from prior work on attachment recognition in children, was compared against Logistic Regression and a Support Vector Machine over the Androids Corpus, with all models trained on an identical set of IS09 acoustic features under the five person-independent folds distributed with the corpus.

The temporal classifier achieved its best performance at the shortest segment length tested, approximately one second of speech, reaching 73.5\% accuracy and 71.4\% F1. This exceeded the matched Logistic Regression baseline by 2.84 F1 points, but the difference was not statistically significant at any segment length, under either aggregation strategy. Within this experimental setup, therefore, the capacity to model temporal structure gave no measurable advantage over classifying frames independently. Two caveats attach to that conclusion. The first is its scope: it holds for one architecture, one feature representation and one corpus, and a different representation might well yield a different answer. The second is statistical: with only five folds, a difference of the size observed would not reliably have been detected even if it were real, and the absence of a significant result is not evidence that no difference exists.
...
Future research directions emerge directly from these limitations. The comparison could be repeated with self-supervised speech embeddings in place of handcrafted features, which would test whether the null result reflects the architecture or the representation; with gated recurrent architectures, which handle longer sequences more effectively than the model used here; and with segmentation confined within turns, removing the discontinuity introduced by concatenation. The contribution of the interviewer, excluded from the primary condition here, is also worth examining as a question in its own right, since the interviewer's behaviour may itself carry information about the participant's condition.

\appendix % first appendix
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Complete Results}\label{app:full-results}

This appendix reports the full set of results summarised in Chapter~\ref{results}. Tables~\ref{tab:appendix-metrics-participant} and~\ref{tab:appendix-metrics-full} give the classification metrics for every model and segment length under each audio condition, and Tables~\ref{tab:appendix-stats-participant} and~\ref{tab:appendix-stats-full} give the corresponding statistical comparisons.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llcccc}
    \toprule
    Model & $L$ & Accuracy & Precision & Recall & F1 \\
    \midrule
    % \input{tab_a1}   % rows generated from summary_participant_speech_only.csv
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[htbp]
  \centering
  \caption{Complete results, full interview recording. All values are
  percentages, reported as the mean and standard deviation over the five folds.}
  ...
  \begin{tabular}{llcccc}
    ...
    Model & $L$ & Accuracy & Precision & Recall & F1 \\
    \midrule
    % \input{tab_a2}   % rows generated from summary_full_interview_recording.csv
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llcccc}
    \toprule
    $L$ & Comparison & $\Delta$F1 & $t$ & $p$ & $\alpha$ \\
    \midrule
    % \input{tab_a3}   % rows generated from statistics_participant_speech_only.csv
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llcccc}
    ...
    $L$ & Comparison & $\Delta$F1 & $t$ & $p$ & $\alpha$ \\
    \midrule
    % \input{tab_a4}   % rows generated from statistics_full_interview_recording.csv
    \bottomrule
  \end{tabular}
\end{table}
